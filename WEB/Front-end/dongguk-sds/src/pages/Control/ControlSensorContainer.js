...
function ControlSensorContainer() {
  ...
  const [SensorData, setSensorData] = useState(ControlSensorData);
  ...
  function filterList() {
    return SensorData.filter((ele) => ele.sensor_name.includes(searchName) && ele.room_name.includes(filterLocation));
  }
